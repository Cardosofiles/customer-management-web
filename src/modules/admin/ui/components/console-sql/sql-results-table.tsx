...
const ResultContent = ({ result }: { result: SqlQueryResult }): JSX.Element => {
  ...
  // ── INSERT / UPDATE / DELETE — sem rows retornadas ─────────────────────────────────────────────
  if (!rows?.length) {
    return (
      <div
        aria-live="polite"
        className="flex items-center gap-2 p-4 text-xs uppercase tracking-[0.2em] text-muted-foreground"
      >
        <span className="text-emerald-500" aria-hidden>
          ✓
        </span>
        <span>{result.message ?? `${rowCount ?? 0} linhas afetadas`}</span>
  ...
  return (
    <div className="flex h-full flex-col gap-2 p-3" aria-live="polite">
      <div className="flex shrink-0 items-center justify-between">
        {/* ✅ usa result.message (ex: "5 linhas retornadas") */}
        <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">
          {result.message ?? `${rowCount ?? rows.length} linhas`} · {formatDuration(duration)}
        </p>
        {isTruncated && (
      ...
      <div className="min-h-0 flex-1 overflow-auto border border-border/60">
        <div className="inline-block min-w-max">
          <Table
            aria-label={`Resultado da query — ${result.message ?? `${rowCount ?? rows.length} linhas`}`}
